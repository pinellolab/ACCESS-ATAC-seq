...
def get_norm_signal(bw_raw, bw_bias, chrom, start, end, half_window, pseudo_count):
    ...
    for i in range(half_window, len(signal_raw) - half_window):
        ...
        if total_bias == 0:
            ...
            signal_exp[i - half_window] = total_signal * signal_bias[i] / total_bias

    # normalized signal = obs/exp
    signal_norm = np.divide(
        signal_raw[half_window:-half_window] + pseudo_count, signal_exp + pseudo_count
    )
...
def main():
    ...
    dat = np.empty(shape=(len(grs), 201, 5), dtype=np.float32)
    ...
    for i, (chrom, start, end) in enumerate(zip(grs.Chromosome, grs.Start, grs.End)):
        ...
        signal_total = np.sum(np.array(bw_raw.values(chrom, start, end)), keepdims=True)
        signal = np.concatenate([signal_normal, signal_total])
        signal = np.expand_dims(signal, axis=1)

        seq = str(fasta.fetch(chrom, start, end)).upper() + "N"
        x = one_hot_encode(seq=seq)

        # assemble data
        dat[i] = np.concatenate([x, signal], axis=1)
